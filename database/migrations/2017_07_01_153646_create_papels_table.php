    ...
    public function up()
    {
        Schema::create('papels', function (Blueprint $table) {
            ...
            $table->timestamps();
                        ...
                        // O nome relciona os models
        Schema::create('papel_permissao', function (Blueprint $table) {
            // N para N
            $table->integer('permissao_id')->unsigned();
            $table->integer('papel_id')->unsigned();

            $table->foreign('permissao_id')->references('id')
            ->on('permissaos')->onDelete('cascade');

            $table->foreign('papel_id')->references('id')
            ->on('papels')->onDelete('cascade');
